...
const flirting = async (m, Fox) => {
  ...
  if (validCommands.includes(cmd)) {
    try {
      ...
      await Fox.sendMessage(m.from, { text: result, mentions: [m.sender] }, { quoted: m });
    } catch (error) {
      console.error('Error fetching flirt message:', error);
      await Fox.sendMessage(m.from, { text: "Failed to retrieve flirt message. Please try again later." });
    }
  }
};
