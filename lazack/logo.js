...
zokou({ nomCom: "hacker",
    ...
    if (!arg || arg == "") {
        repondre("*__Example : * " + prefixe + "hacker Lazack");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        ...
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t Logo by Lazack-Md*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        const noArgMsg = `*_EXAMPLE *:  ${prefixe}dragonball Lazack`;
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        if (!arg || arg == '') {
            repondre("*_Example : * " + prefixe + "naruto Lazack");
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by Lazack-Md *" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
repondre("  Lazack is Processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by Lazack-Md*"})
