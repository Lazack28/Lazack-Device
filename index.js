...
// === FIXED SECTION: DIRECT HANDLER ATTACHMENT ===
console.log(chalk.bold.cyan('\n=== ATTACHING MESSAGE HANDLER ==='))
...
if (originalHandler && typeof originalHandler === 'function') {
  console.log(chalk.bold.green('✅ Handler function found'))
  ...
  const messageHandler = async (chatUpdate) => {
    try {
      ...
      if (chatUpdate?.messages?.length > 0) {
        ...
        console.log(chalk.bold.cyan('From:'), remoteJid)
        console.log(chalk.bold.cyan('From Me:'), isFromMe)
        console.log(chalk.bold.cyan('Type:'), messageType)
        ...
        if (text) {
          console.log(chalk.bold.yellow('Text:'), text)
          
          // Check for command
          const prefix = '!' // Default prefix
          if (text.startsWith(prefix)) {
            console.log(chalk.bold.green('✅ COMMAND DETECTED!'))
          }
        }
      }
      
      // Call the actual handler
      await originalHandler.call(global.conn, chatUpdate)
  ...
  try {
    ...
    console.log(chalk.bold.yellow('Removed existing listeners'))
  } catch (e) {
    console.log(chalk.bold.yellow('No listeners to remove'))
  }
  
  // Attach the handler
  global.conn.ev.on('messages.upsert', messageHandler)
  
  console.log(chalk.bold.green('✅ Message handler attached successfully!'))
  console.log(chalk.bold.cyan('Event listeners count:'), global.conn.ev.listenerCount('messages.upsert'))
  
} else {
  console.log(chalk.bold.red('❌ ERROR: Handler function not found or invalid!'))
  console.log(chalk.bold.yellow('Handler type:'), typeof originalHandler)
}

// Also attach connection update handler
global.conn.ev.on('connection.update', connectionUpdate)

console.log(chalk.bold.green('\n✅ Bot initialization complete!'))
console.log(chalk.bold.yellow('🤖 Send "!ping" to test commands'))
console.log(chalk.bold.cyan('📊 Plugins loaded:'), Object.keys(global.plugins).length)
console.log(chalk.bold.magenta('🎯 Waiting for messages...\n'))

// Call reloadHandler to ensure everything is set up
console.log(chalk.bold.cyan('Calling reloadHandler...'))
try {
  await global.reloadHandler()
  console.log(chalk.bold.green('✅ reloadHandler completed successfully'))
} catch (e) {
